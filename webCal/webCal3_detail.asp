...
Set rs = db.Execute(query,,&H0001)
	...
	if rs("time_start") <> "" AND rs("time_end") <> "" then
		startTime = TimeValue(rs("time_start"))
		endTime = TimeValue(rs("time_end"))
	end if
	userID = rs("user_id")
rs.Close
Set rs = nothing
...
<tr>
	...
	<tr>
		<td bgcolor="#<%=color(4)%>">
			...
			<a href="webCal3_<%=Request.QueryString("view")%>.asp?date=<%=eventDate%>"
...
if Session(dataName & "User") <> userID then
	response.write "<a href=""webCal3_login.asp?url=" & url
%>
...
<% if CInt(Session(dataName & "User")) = CInt(userID) then %>
